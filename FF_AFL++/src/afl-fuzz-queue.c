...
void update_bitmap_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  if (afl->shm.fishfuzz_mode && afl->shm.fish_map) update_bitmap_score_target(afl, q);
...
void cull_queue_explore(afl_state_t *afl) {
  ...
  for (i = 0; i < FUNC_SIZE; i++) {
    if (afl->top_rated_explore[i] && !afl->virgin_funcs[i]) {
...
void cull_queue_exploit(afl_state_t *afl) {
  ...
  if (afl->non_instrumented_mode || (!afl->target_changed && !should_skip)) return;
    ...
    if (temp_v[i >> 3] & (1 << (i & 7)) && afl->reach_bits_count[i]
      ...
      while (k--) 
        if (selected->trace_target[k]) 
          temp_v[k] &= ~selected->trace_target[k];
  ...
  if (afl->current_targets_triggered) avg_violation_trigger = total_trigger_cnt / afl->current_targets_triggered;
  if (afl->current_targets_reached) avg_violation_visit = total_visit_cnt / afl->current_targets_reached;
...
/* 
  ...
  do selection among three stages
...
void cull_queue(afl_state_t *afl) {
  ...
  if (afl->fish_seed_selection == INTER_FUNC_EXPLORE) {
    ...
    if (get_cur_time() - afl->last_func_time < INTER_EXPLORE_TLIMIT && !afl->skip_inter_func) {
      ...
      if (get_cur_time() - afl->last_reach_time > INTRA_EXPLORE_TLIMIT || should_skip) {
        afl->target_changed = 1;
        afl->last_trigger_time = get_cur_time();
        afl->fish_seed_selection = TARGET_EXPLOIT;
