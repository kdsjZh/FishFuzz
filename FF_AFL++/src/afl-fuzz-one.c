...
static u8 is_too_large(afl_state_t *afl, struct queue_entry *q) {
  ...
  u64 eff_limit = SKIP_DET_US_TLIMIT * afl->blocks_eff_total / afl->blocks_eff_select;

  if (det_power * q->exec_us > eff_limit && get_cur_time() - afl->start_time > (1000U * 3600)) {
...
abandon_entry:
  ...
  if (!afl->stop_soon && !afl->queue_cur->cal_failed &&
    ...
    afl->reinit_table = 1;
    if (afl->queue_cur->favored) { --afl->pending_favored; ++afl->queued_fuzzed_favored; }
    else ++afl->queued_fuzzed_non_favored;
